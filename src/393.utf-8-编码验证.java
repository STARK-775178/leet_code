...
class Solution {
    public boolean validUtf8(int[] data) {
        /**
         * Accepted
         * 52/52 cases passed (101 ms)
         * Your runtime beats 5.12 % of java submissions
         * Your memory usage beats 11.02 % of java submissions (42.6 MB)
         */
        int count = 0;
        for (int i = 0; i < data.length; i++) {
            String str = Integer.toBinaryString(data[i]);
            // 如果str的长度不是8位，那么在前面补0
            if (str.length() != 8) {
            ...
            if (count == 0) {
                count = numsOfUtf(str);
                if (count == 1 || count > 4||data.length - i < count) {
                    return false;
                }
                if (count == 0)
                    continue;
    ...
    private int numsOfUtf(String str0) {
        int count = 0;
