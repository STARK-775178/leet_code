...
class Solution {
    class Segment {
        ...
        public Segment(long start, long end, long color) {
            this.start = start;
            this.end = end;
            this.color = color;
        }
    ...
    public List<List<Long>> splitPainting(int[][] segments) {
        ...
        for (int[] segment : segments) {
            segmentList.add(new Segment(segment[0], segment[1], segment[2]));
        }

        Collections.sort(segmentList, (a, b) -> {
            if (a.start == b.start) {
        ...
        for (Segment segment : segmentList) {
            map.put(segment.start, map.getOrDefault(segment.start, 0L) + segment.color);
            map.put(segment.end, map.getOrDefault(segment.end, 0L) - segment.color);
        }
        ...
        for (Entry<Long, Long> entrySet : map.entrySet()) {
            Long end = entrySet.getKey();
            Long newColor = entrySet.getValue();
